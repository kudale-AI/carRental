import React from 'react';

const testimonials = [
  {
    name: 'Aman Gupta',
    place: 'Indore',
    image: 'https://images.unsplash.com/photo-1633332755192-727a05c4013d?q=80&w=100',
    message: "good service ,i had a great experience using thier car  for my trip . The car  made trip comfortable and easy to drive   saving me time and effort. Highly recommend it to fellow users!",
  },
  {
    name: 'Shubham Jat',
    place: 'Ujjain',
    image: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=100',
    message: "I've been using imagify for nearly two years, primarily for Instagram, and it has been incredibly user-friendly, making my work much easier.",
  },
  {
    name: 'Disha Chouhan',
    place: 'Gawalior',
    image: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=100&h=100&auto=format&fit=crop',
    message: "I've been using imagify for nearly two years, primarily for Instagram, and it has been incredibly user-friendly, making my work much easier.",
  },
];

const Banner = () => {
  return (
   <>
    <h2 className="text-3xl font-semibold text-center mb-10 mt-16">
      What our users are saying
    </h2>,
    <div className="flex flex-wrap items-center justify-center gap-6">
      {testimonials.map((testimonial, index) => (
        <div
          key={index}
          className="text-sm w-80 border border-gray-200 pb-6 rounded-lg bg-white shadow-[0px_4px_15px_0px] shadow-black/5 overflow-hidden"
        >
          {/* Header */}
          <div className="flex items-center gap-4 px-5 py-4 bg-red-500/10">
            <img
              className="h-12 w-12 rounded-full"
              src={testimonial.image}
              alt={`${testimonial.name} profile`}
            />
            <div>
              <h1 className="text-lg font-medium text-gray-800">{testimonial.name}</h1>
              <p className="text-gray-800/80">{testimonial.place}</p>
            </div>
          </div>

          {/* Stars & Message */}
          <div className="p-5 pb-7">
            <div className="flex gap-0.5">
              {[...Array(5)].map((_, starIndex) => (
                <svg
                  key={starIndex}
                  width="22"
                  height="20"
                  viewBox="0 0 22 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M10.525.464a.5.5 0 0 1 .95 0l2.107 6.482a.5.5 0 0 0 .475.346h6.817a.5.5 0 0 1 .294.904l-5.515 4.007a.5.5 0 0 0-.181.559l2.106 6.483a.5.5 0 0 1-.77.559l-5.514-4.007a.5.5 0 0 0-.588 0l-5.514 4.007a.5.5 0 0 1-.77-.56l2.106-6.482a.5.5 0 0 0-.181-.56L.832 8.197a.5.5 0 0 1 .294-.904h6.817a.5.5 0 0 0 .475-.346z"
                    fill="#FF532E"
                  />
                </svg>
              ))}
            </div>
            <p className="text-gray-500 mt-5">{testimonial.message}</p>
          </div>

          <a href="#" className="text-red-500 underline px-5">
            Read more
          </a>
        </div>
      ))}
    </div>
    </>
  );
};

export default Banner;
